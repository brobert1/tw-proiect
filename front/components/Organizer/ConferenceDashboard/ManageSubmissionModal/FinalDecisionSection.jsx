import { updatePaperDecision } from '@api/organizer';
import { useMutation } from '@hooks';
import { Check, X } from 'lucide-react';

const FinalDecisionSection = ({ conferenceId, paperId, paperStatus, reviewDeadline, onClose }) => {
  const isBeforeDeadline = reviewDeadline ? new Date() < new Date(reviewDeadline) : false;
  const isDecided = paperStatus === 'accepted' || paperStatus === 'rejected';

  const mutation = useMutation((decision) => updatePaperDecision(conferenceId, paperId, decision), {
    invalidateQueries: [`organizer/conferences/${conferenceId}/submissions`],
    successCallback: () => {
      onClose?.();
    },
  });

  const handleDecision = (decision) => {
    if (window.confirm(`Are you sure you want to ${decision} this paper?`)) {
      mutation.mutate(decision);
    }
  };

  return (
    <div className="rounded-lg border border-border-primary bg-white p-4 md:p-6">
      <h3 className="mb-4 text-base font-semibold text-text-primary">Final Decision</h3>
      {isDecided ? (
        <div
          className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium ${
            paperStatus === 'accepted' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
          }`}
        >
          {paperStatus === 'accepted' ? (
            <>
              <Check className="h-4 w-4" /> Paper Accepted
            </>
          ) : (
            <>
              <X className="h-4 w-4" /> Paper Rejected
            </>
          )}
        </div>
      ) : (
        <>
          {isBeforeDeadline && (
            <p className="mb-3 text-sm text-yellow-600">
              Final decision will be available after the review deadline passes.
            </p>
          )}
          <div className="flex flex-col gap-3 md:flex-row md:flex-wrap">
            <button
              type="button"
              disabled={isBeforeDeadline || mutation.isPending}
              onClick={() => handleDecision('accepted')}
              className="inline-flex items-center gap-2 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <Check className="h-4 w-4" />
              {mutation.isPending ? 'Processing...' : 'Accept Paper'}
            </button>
            <button
              type="button"
              disabled={isBeforeDeadline || mutation.isPending}
              onClick={() => handleDecision('rejected')}
              className="inline-flex items-center gap-2 rounded-md bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <X className="h-4 w-4" />
              {mutation.isPending ? 'Processing...' : 'Reject Paper'}
            </button>
          </div>
        </>
      )}
    </div>
  );
};

export default FinalDecisionSection;
